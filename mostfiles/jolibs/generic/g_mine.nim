...
proc replaceAllStrings(tekst: string, replacablesq: seq[array[2, string]]): string =
#[
  Replace all occurences of the strings of replacablesq with their substitues in the tekst.
]#
  ...
  for stringar in replacablesq:
    while stringar[0] in vtekst:
      vtekst = vtekst.replace(stringar[0], stringar[1])

  result = vtekst



proc removeDuplicateStrings_shitty(tekst: string, removablesq: seq[string]): string =
#[ 
...
#[ 
  ...
  result = tekst
  for removest in removablesq:
    if removest.len == 0: continue
    let duplist = removest & removest
    while duplist in result:
      result = result.replace(duplist, removest)



proc removeMostWhiteSpace*(tekst: string, typeit: int): string = 

  var newtekst: string
  case typeit
  of 0:
    result = removeDuplicateStrings(tekst, @[" ", "\n", "\t","\c", "\c\n" ,"\n ", "\t\n", "\t\c", "\t \n"])
  of 1:
    newtekst = removeSingleStrings(tekst, @["\t\p"])
    newtekst = removeDuplicateStrings(newtekst, @[" ", " \t", " \p" ,"\p ", "\t \p"])
    newtekst = replaceAllStrings(newtekst, @[["\p \p", "\p\p"], ["\p\p\p", "\p\p"]])
    result = removeDuplicateStrings(newtekst, @[" ", " \t", " \p" ,"\p ", "\t \p"])
  else:
    raise newException(ValueError, "removeMostWhiteSpace encountered an invalid type-number (typeit)")
...
#[  ]#
  ...
  echo "-------------------------------------------------"
  echo removeDuplicateStrings("aap \p \p \p \p noot", @[" \p"])
